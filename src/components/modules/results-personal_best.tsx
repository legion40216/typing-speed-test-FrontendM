import iconCelebrate from "../../assets/icon-new-pb.svg";
import { RefreshCcwIcon } from "lucide-react";
import { Button } from "../ui/button";

interface ResultsPersonalBestProps {
wpm: number;
accuracy: number;
timeTaken: number;
correctChars: number;
...
export default function ResultsPersonalBest({
  wpm,
  accuracy,
  timeTaken,
  correctChars,
  incorrectChars,
  onReset
}:ResultsPersonalBestProps) {
  
  return (
    <div className="flex flex-col items-center justify-center p-6 space-y-20">
      <div className="flex flex-col items-center space-y-4">
        <img src={iconCelebrate} alt="New personal best celebration" />
        <h1 className="text-4xl font-bold mt-4 text-style-neutral-0">High score smashed</h1>
        <p className="text-2xl text-style-neutral-400">You are getting faster!, incredible typing</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-style-neutral-400">
        ...
        <div className="flex flex-col border border-style-neutral-400 rounded-lg p-4 aspect-video">
          <h2>Accuracy</h2>
          <p className={`font-bold ${accuracy >= 90 ? "text-style-green-500" : "text-style-red-500"}`}>
            {accuracy.toFixed(2)}%
          </p>
        </div>
